...
const VEHICLE_JOB_STATUS = {
  NEW: 'neu',
  AT_ADVISOR: 'bei_serviceberater',
  READY_FOR_DETAILING: 'bereit_fuer_reinigung',
  IN_DETAILING: 'in_reinigung',
  READY_FOR_DISPO: 'bereit_fuer_auslieferung',
  COMPLETED: 'abgeschlossen'
};
const VEHICLE_JOB_STATUS_LABELS = {
  [VEHICLE_JOB_STATUS.NEW]: 'Neu',
  [VEHICLE_JOB_STATUS.AT_ADVISOR]: 'Beim Serviceberater',
  [VEHICLE_JOB_STATUS.READY_FOR_DETAILING]: 'Bereit für Reinigung',
  [VEHICLE_JOB_STATUS.IN_DETAILING]: 'In Reinigung',
  [VEHICLE_JOB_STATUS.READY_FOR_DISPO]: 'Bereit für Auslieferung',
  [VEHICLE_JOB_STATUS.COMPLETED]: 'Abgeschlossen'
};

function vehicleJobSelectSql() {
  return `
    SELECT vj.*,
      advisor.username AS advisor_username,
      advisor.display_name AS advisor_display_name,
      creator.username AS created_by_username
    FROM vehicle_jobs vj
    LEFT JOIN users advisor ON advisor.id = vj.advisor_user_id
    LEFT JOIN users creator ON creator.id = vj.created_by_user_id
  `;
}
...
function mapVehicleJob(row) {
  ...
  return {
    ...
    advisor: row.advisor_user_id ? {
      ...
      username: row.advisor_username || '',
      display_name: row.advisor_display_name || row.advisor_username || ''
    } : null,
    created_by_username: row.created_by_username || ''
  };
}

function getAdvisorUsers() {
  return db.prepare(`
    SELECT DISTINCT u.id, u.username, COALESCE(NULLIF(u.display_name, ''), u.username) AS display_name
    FROM users u
    LEFT JOIN user_module_permissions ump ON ump.user_id = u.id
    WHERE COALESCE(u.active, 1) = 1
      AND (
        u.role = 'admin'
        OR (ump.module_key = ? AND (ump.can_view = 1 OR ump.can_edit = 1 OR ump.can_manage = 1))
      )
    ORDER BY display_name
  `).all(MODULES.ADVISOR);
}

function vehicleJobHistory(jobId) {
  return db.prepare(`
    SELECT vjh.*, COALESCE(NULLIF(u.display_name, ''), u.username, '') AS changed_by_display
    FROM vehicle_job_history vjh
    LEFT JOIN users u ON u.id = vjh.changed_by_user_id
    WHERE vjh.vehicle_job_id = ?
    ORDER BY vjh.created_at DESC, vjh.id DESC
  `).all(jobId);
}

function addVehicleJobHistory(vehicleJobId, status, note, changedByUserId = null) {
  db.prepare(`
    INSERT INTO vehicle_job_history (vehicle_job_id, status, note, changed_by_user_id)
    VALUES (?, ?, ?, ?)
  `).run(vehicleJobId, status, String(note || ''), changedByUserId ? Number(changedByUserId) : null);
}
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS vehicle_job_history (
    ...
    created_at TEXT DEFAULT (datetime('now')),
    FOREIGN KEY(vehicle_job_id) REFERENCES vehicle_jobs(id) ON DELETE CASCADE,
...
ensureColumn('vehicle_jobs', 'pickup_address', "TEXT DEFAULT ''");
ensureColumn('vehicle_jobs', 'delivery_address', "TEXT DEFAULT ''");
...
ensureColumn('vehicle_jobs', 'created_by_user_id', 'INTEGER');
ensureColumn('vehicle_jobs', 'updated_by', "TEXT DEFAULT ''");
...
ensureColumn('vehicle_job_history', 'changed_by_user_id', 'INTEGER');
...
app.get('/api/meta', auth, (req, res) => {
  ...
  if (can(req.user, MODULES.DISPO, 'view')) {
    ...
    response.advisors = getAdvisorUsers();
  }
  if (can(req.user, MODULES.ADVISOR, 'view')) {
    response.advisors = getAdvisorUsers();
  }
  res.json(response);
});
...
app.get('/api/vehicle-jobs', auth, (req, res) => {
  ...
  let sql = `${vehicleJobSelectSql()} WHERE 1=1`;
  const params = [];
  const scope = String(req.query.scope || 'all');

  if (can(req.user, MODULES.ADVISOR, 'view') && !can(req.user, MODULES.DISPO, 'view')) {
    sql += ' AND vj.advisor_user_id = ?';
    params.push(req.user.id);
  }
  if (scope === 'advisor' && can(req.user, MODULES.ADVISOR, 'view')) {
    sql += ' AND vj.advisor_user_id = ?';
    params.push(req.user.id);
  }
  if (scope === 'detailing') {
    sql += ' AND vj.cleaning_required = 1 AND vj.status IN (?, ?)';
    params.push(VEHICLE_JOB_STATUS.READY_FOR_DETAILING, VEHICLE_JOB_STATUS.IN_DETAILING);
  } else if (scope === 'dispo_ready') {
    sql += ' AND vj.status = ?';
    params.push(VEHICLE_JOB_STATUS.READY_FOR_DISPO);
  } else if (scope === 'open') {
    sql += ' AND vj.status != ?';
    params.push(VEHICLE_JOB_STATUS.COMPLETED);
  }

  sql += " ORDER BY CASE WHEN vj.deadline = '' THEN 1 ELSE 0 END, vj.deadline ASC, vj.updated_at DESC, vj.id DESC";
  const rows = db.prepare(sql).all(...params).map(mapVehicleJob);
  const withHistory = rows.map(row => ({ ...row, history: vehicleJobHistory(row.id) }));
...
app.post('/api/vehicle-jobs', auth, (req, res) => {
  ...
  const vehicleLabel = String(b.vehicle_label || '').trim();
  if (!customerName || !plate || !vehicleLabel) return res.status(400).json({ error: 'Kunde, Kennzeichen und Fahrzeug sind erforderlich' });
  const advisorId = b.advisor_user_id ? Number(b.advisor_user_id) : null;
  if (advisorId && !db.prepare('SELECT id FROM users WHERE id = ?').get(advisorId)) return res.status(400).json({ error: 'Serviceberater nicht gefunden' });
  const status = String(b.status || (req.user.role === 'admin' || can(req.user, MODULES.DISPO, 'edit') ? VEHICLE_JOB_STATUS.NEW : VEHICLE_JOB_STATUS.AT_ADVISOR));
  db.prepare(`
    INSERT INTO vehicle_jobs (
      ...
      cleaning_required, cleaning_type, deadline, status, notes,
      created_by_user_id, updated_by
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    customerName, plate, vehicleLabel, String(b.phone || '').trim(), advisorId,
    b.hb_required ? 1 : 0, b.pickup_required ? 1 : 0, b.delivery_required ? 1 : 0,
    String(b.pickup_address || '').trim(), String(b.delivery_address || '').trim(),
    b.cleaning_required ? 1 : 0, String(b.cleaning_type || '').trim(), String(b.deadline || '').trim(),
    Object.values(VEHICLE_JOB_STATUS).includes(status) ? status : VEHICLE_JOB_STATUS.NEW,
    String(b.notes || '').trim(), req.user.id, req.user.display_name || req.user.username
  );
  const id = db.prepare('SELECT last_insert_rowid() AS id').get().id;
  addVehicleJobHistory(id, status, 'Vorgang angelegt', req.user.id);
  const row = mapVehicleJob(db.prepare(`${vehicleJobSelectSql()} WHERE vj.id = ?`).get(id));
  const payload = { ...row, history: vehicleJobHistory(id) };
  broadcast('vehicle_job_updated', payload);
  res.json(payload);
});
...
app.put('/api/vehicle-jobs/:id', auth, (req, res) => {
  ...
  const id = Number(req.params.id);
  const existing = db.prepare('SELECT * FROM vehicle_jobs WHERE id = ?').get(id);
  if (!existing) return res.status(404).json({ error: 'Vorgang nicht gefunden' });
  if (can(req.user, MODULES.ADVISOR, 'view') && !can(req.user, MODULES.DISPO, 'edit')) {
    if (existing.advisor_user_id !== req.user.id) return res.status(403).json({ error: 'Nicht dein Fahrzeugvorgang' });
  }
  const b = req.body || {};
  const customerName = String(b.customer_name || '').trim();
  const plate = String(b.plate || '').trim();
  ...
  db.prepare(`
    UPDATE vehicle_jobs SET
      ...
      hb_required = ?, pickup_required = ?, delivery_required = ?,
      pickup_address = ?, delivery_address = ?,
      cleaning_required = ?, cleaning_type = ?, deadline = ?, notes = ?, updated_by = ?, updated_at = datetime('now')
    WHERE id = ?
  `).run(
    customerName, plate, vehicleLabel, String(b.phone || '').trim(), advisorId,
    b.hb_required ? 1 : 0, b.pickup_required ? 1 : 0, b.delivery_required ? 1 : 0,
    String(b.pickup_address || '').trim(), String(b.delivery_address || '').trim(),
    b.cleaning_required ? 1 : 0, String(b.cleaning_type || '').trim(), String(b.deadline || '').trim(),
    String(b.notes || '').trim(), req.user.display_name || req.user.username, id
  );
  addVehicleJobHistory(id, existing.status, 'Vorgang bearbeitet', req.user.id);
  const row = mapVehicleJob(db.prepare(`${vehicleJobSelectSql()} WHERE vj.id = ?`).get(id));
  const payload = { ...row, history: vehicleJobHistory(id) };
  broadcast('vehicle_job_updated', payload);
  res.json(payload);
...
app.post('/api/vehicle-jobs/:id/status', auth, (req, res) => {
  ...
  if (!Object.values(VEHICLE_JOB_STATUS).includes(status)) return res.status(400).json({ error: 'Ungültiger Status' });

  // Role-based transitions
  const isDispo = can(req.user, MODULES.DISPO, 'edit');
  const isAdvisor = can(req.user, MODULES.ADVISOR, 'edit');
  const isDetailing = can(req.user, MODULES.DETAILING, 'edit');
  let allowed = false;
  if (isDispo && [VEHICLE_JOB_STATUS.NEW, VEHICLE_JOB_STATUS.AT_ADVISOR, VEHICLE_JOB_STATUS.READY_FOR_DISPO, VEHICLE_JOB_STATUS.COMPLETED].includes(status)) allowed = true;
  if (isAdvisor && existing.advisor_user_id === req.user.id && [VEHICLE_JOB_STATUS.AT_ADVISOR, VEHICLE_JOB_STATUS.READY_FOR_DETAILING, VEHICLE_JOB_STATUS.READY_FOR_DISPO].includes(status)) allowed = true;
  if (isDetailing && [VEHICLE_JOB_STATUS.IN_DETAILING, VEHICLE_JOB_STATUS.READY_FOR_DISPO].includes(status)) allowed = true;
  if (!allowed && req.user.role !== 'admin') return res.status(403).json({ error: 'Statuswechsel nicht erlaubt' });

  db.prepare(`UPDATE vehicle_jobs SET status = ?, updated_by = ?, updated_at = datetime('now') WHERE id = ?`)
    .run(status, req.user.display_name || req.user.username, id);
  addVehicleJobHistory(id, status, String((req.body || {}).note || ''), req.user.id);
  const row = mapVehicleJob(db.prepare(`${vehicleJobSelectSql()} WHERE vj.id = ?`).get(id));
  const payload = { ...row, history: vehicleJobHistory(id) };
  broadcast('vehicle_job_updated', payload);
  res.json(payload);
